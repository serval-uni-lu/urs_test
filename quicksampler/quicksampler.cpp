...
public:
    QuickSampler(std::string input, int max_samples, double max_time, bool use_seed, unsigned int seed) : opt(c), input_file(input), max_samples(max_samples), max_time(max_time), m_use_seed(use_seed), m_seed(seed) {}
...
int main(int argc, char * argv[]) {
    ...
    QuickSampler s(argv[argc-1], max_samples, max_time, use_seed, seed);
    s.run();
    return 0;
}
