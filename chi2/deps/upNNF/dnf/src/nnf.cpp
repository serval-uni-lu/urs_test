...
void ANNF::set_assumps(LitSet const& a) {
    assumps = a;
}

void ANNF::clear_assumps() {
    assumps.clear();
}
...
void ANNF::get_solution(mpz_int const& id, LitSet & s) const {
    std::vector<std::pair<mpz_int, std::size_t> > stack;
    stack.push_back(std::pair(id, NNF::ROOT));
