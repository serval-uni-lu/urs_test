...
int   MultiComponents::getComponent(int x)             { return find(x);             }
int   MultiComponents::getStart(int x)                 { return start[find(x)];      }
int   MultiComponents::getCompSize(int x)              { return length[find(x)];     }
int   MultiComponents::getNumComponents()              { return numMultiComponents;  }
int   MultiComponents::getMaxLength()                  { return maxLength;           }
...
bool MultiComponents::isOutOfSync() { return outofsync;  }
void MultiComponents::sync()        { outofsync = false; }
...
float MultiComponents::joinEntropy(int x, int y) {
    ...
    float pr1 = length[xRoot]/(float)var2pos.size();
    float pr2 = length[yRoot]/(float)var2pos.size();
    ...
    parent[other]  = newRep;
    length[newRep] += length[other];
    if (length[newRep] > maxLength)
        maxLength = length[newRep];
    

    if (start[other] < start[newRep])
        start[newRep] = start[other];
    
    float pr = length[newRep]/(float)var2pos.size();
    res -= pr*log2(pr);
    return res;
}
...
void MultiComponents::vectorJoin(std::unordered_set<int>& vec) {
    std::unordered_set<int>::iterator its1 = vec.begin();
    for(std::unordered_set<int>::iterator its2 = its1; its2 != vec.end(); its2++)
            if (its1 != its2)
                makeUnion(*its1, *its2, false);
...
bool MultiComponents::internalJoin(synExp* s, bool changeOrder) {
    ...
    //std::cerr << std::endl;
    //printComponents();
    std::unordered_set<int>::const_iterator its1 = s->giveSymbolIndices().begin();
    for(std::unordered_set<int>::const_iterator its2 = its1; its2 != s->giveSymbolIndices().end(); its2++)
            if (its1 != its2) {
...
void MultiComponents::reorder() {
    ...
    for(int i : pos2var) {
        h.push_back(orderHelper(i, var2pos[i], start[find(i)], avg[find(i)]));
    }
    ...
    while (pos < pos2var.size()) {
        ...
        start[find(pos2var[pos])] = pos;
        pos += length[find(pos2var[pos])];
    }
    //check();
