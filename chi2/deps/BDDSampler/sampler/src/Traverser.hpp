...
class Traverser {
    ...
    std::vector<T> traverseVector() {
        ...
        for(auto headNode : adapter->getFunctionHeads())
            trav_recur1(headNode);
        ...
        for(auto headNode : adapter->getFunctionHeads()) {
            TravInfo<T>& infoHead = storage[headNode];
            if (!infoHead.getMark())
                trav_recur2(headNode);
            
            T* t  = storage[headNode].getRes();
            int rindex = adapter->getLevel(headNode);
            // It makes no sense to call a extendedCombiner if the node
            // does not exist
            if (fakeZeroLevel && rindex != 0)
                if (!extendedCombiner)
                    res.push_back(combine(0, rindex, rindex, *t, *t));
                else // We make up a node pointer and hope for the best
                    res.push_back(extcombine(0, rindex, rindex, *t, *t, 0));
                else
                    res.push_back(T(*t));
            incVisit(headNode);
        }
    ...
    std::vector<T> traverseMPVector(int numThreads) {
        if (numThreads <= 0) {
            std::cerr << "Wrong number of threads " << numThreads << std::endl;
            exit(-1);
        }
        std::vector<T> res;
        // First go, traverse BDD to count references
        ...
        for(auto headNode : adapter->getFunctionHeads())
            fillBuffer(headNode, numThreads);
...
private:
    ...
    // The only thing we do here is compute how many
    // references a node has
...
std::vector<T> traverseVector(cuddAdapter* adapter,
              T resZero,
              T resOne,
              T (*combine)(int pindex, int tindex, int eindex,
                           const T& tr, const T& er),
              bool fakeZeroLevel = true) {
                  ...
                  return tr.traverseVector();
              };

// The multithreaded version
template <typename T> inline
std::vector<T> traverseMPVector(int threads,
                                cuddAdapter* adapter,
                                T resZero,
                                T resOne,
                                T (*combine)(int pindex, int tindex, int eindex,
    ...
    return tr.traverseMPVector(threads);
};

// The multithreaded version with a verbose flag
template <typename T> inline
std::vector<T> traverseMP(int threads, int verbose, cuddAdapter* adapter,
             T resZero,
             T resOne,
             T (*combine)(int pindex, int tindex, int eindex,
                          const T& tr, const T& er),
...
template <typename T> inline
std::vector<T> traverse(cuddAdapter* adapter,
           T resZero,
           T resOne,
           T (*extcombine)(int pindex, int tindex, int eindex,
...
template <typename T> inline
std::vector<T> traverse(int verbose,
           cuddAdapter* adapter,
           T resZero,
           T resOne,
           T (*extcombine)(int pindex, int tindex, int eindex,
...
template <typename T> inline
std::vector<T> traverseMP(int threads, cuddAdapter* adapter,
           T resZero,
           T resOne,
           T (*extcombine)(int pindex, int tindex, int eindex,
                           const T& tr, const T& er, DdNode* node),
...
template <typename T> inline
std::vector<T> traverseMP(int verbose,
                          int threads,
                          cuddAdapter* adapter,
                          T resZero,
                          T resOne,
                          T (*extcombine)(int pindex,
                                          int tindex,
                                          int eindex,
                                          const T& tr,
                                          const T& er,
