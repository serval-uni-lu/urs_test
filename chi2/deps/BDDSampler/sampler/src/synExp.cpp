...
synExpT                         synExp::get_type()          const {
      throw std::logic_error("Error accessing get_type in wrong object.\n");
     return synExp_Const;
 }
std::string                     synExp::get_string()        const {
         throw std::logic_error("Error accessing get_string in wrong object.\n");
         return "";           }
...
synSymbol::synSymbol(std::string s) : synExp() {
...
synUnknown::synUnknown(std::string s) : synExp(), st(s)     {};
synExpT           synUnknown::get_type()          const { return synExp_Unknown;  }
std::string       synUnknown::get_string()        const { return st;              }
...
synComment::synComment(std::string s) : synExp(), comment(s)     {};
...
synExp *makeEqual(synExp *e1, synExp *e2, bool assumeBool) {
    ...
    if (e1 == synFalse && e2 == synModule) {
        res = synFalse;
        return res;
    }
    if (e2 == synFalse && e1 == synModule) {
        res = synFalse;
        return res;
    }
...
std::list<synExp *> toCNF_rec(synExp* e) {
    ...
    if (e->isImplies()) {
        std::list<synExp*> t = toCNF_rec(makeOr(makeNot(e->first()->copy_exp()), e->second()->copy_exp()));
        res.insert(res.end(), t.begin(), t.end());
        e->destroy();
        return res;
    }
    ...
    if (e->isIf()) {
        ...
        res.insert(res.end(), t.begin(), t.end());
        if (e->third() != NULL) {
            std::list<synExp*> w = toCNF_rec(makeOr(e->first()->copy_exp(), e->third()->copy_exp()));
...
synExp* synCompound::copy_exp() {
    //std::cerr << "Copying exp " << (synExp*) this << std::endl;
